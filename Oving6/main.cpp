...
public:
  ...
  public:
    ...
    bool valid_move(int from_x, int from_y, int to_x, int to_y) const override {
      return std::abs(from_x - to_x) + std::abs(from_y - to_y) == 1; // Can only move to one step in either x-direction or y-direction
    }

    std::string acronym() const override {
      return (Piece::color_string() == "White" ? "W-KI" : "B-KI");
    }
  };
  ...
  public:
    ...
    bool valid_move(int from_x, int from_y, int to_x, int to_y) const override {
      return std::abs(from_x - to_x) + std::abs(from_y - to_y) == 3; // Can either move 1 step in x + 2 steps in y or move 2 steps in x + 1 step in y
    }

    std::string acronym() const override {
      return (Piece::color_string() == "White" ? "W-KN" : "B-KN");
    }
  };
...
public:
  ChessBoardPrint(ChessBoard &board) {
    ...
    board.after_piece_move = [&board]() {
      ...
      for (string::size_type h = 0; h < letters.size(); ++h)
        cout << "       " << letters.at(h) << "     ";
      cout << endl;
      for (int i = 0; i < 8; ++i) {
        cout << counter;
        for (int j = 0; j < 8; ++j) {
          ...
          piece != nullptr ? cout << "|____" << piece->acronym() + "____" : cout << "|____________";
        }
        cout << "|\n";
        ++counter;
        cout << endl;
